...
describe("Client + InProcessPeerParty", () => {
	it("should allow communication", () => {
		...
		clientA.addOperation({ operationId: "abc1", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "blue" }, timestamp: 0 });
		clientB.addOperation({ operationId: "abc2", data: { metaLevel: 1, type: "recolor", name: "Edit Draw Line", target: "abc1", color: "green" }, timestamp: 1 });
	...
	it("should order operations according to timestamp", () => {
		...
		party.addPeer(clientA);
		party.addPeer(clientB);
		clientA.addOperation({ operationId: "abc1", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "blue" }, timestamp: 1 });
		clientB.addOperation({ operationId: "abc2", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "yellow" }, timestamp: 0 });
		expect(clientA.metaHistory).toEqual([
	...
	it("should order operations according to clientId and order received if timestamps are equal", () => {
		...
		party.addPeer(clientB);
		clientA.addOperation({ operationId: "abc1", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "blue" }, timestamp: 1 });
		clientB.addOperation({ operationId: "abc2", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "yellow" }, timestamp: 1 });
		clientB.addOperation({ operationId: "abc3", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "green" }, timestamp: 1 });
		clientA.addOperation({ operationId: "abc4", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "red" }, timestamp: 1 });
		expect(clientA.metaHistory).toEqual([
			{ clientId: 1, operationId: "abc1", data: { metaLevel: 0, type: "line", name: "Draw Line", color: "blue" }, timestamp: 1 },
	...
	it("should handle updates to continuous operations", () => {
		...
		party.addPeer(clientA);
		party.addPeer(clientB);
		clientA.addOperation({ operationId: "abc1", data: { metaLevel: 0, type: "brush", name: "Brush", points: [{ x: 5, y: 5 }] }, timestamp: 0 });
		...
		expect(clientB.metaHistory[0].data.points[1]).not.toBe(clientA.metaHistory[0].data.points[1]); // must not cheat by copying the reference
