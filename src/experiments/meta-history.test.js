import { IncrementalMetaHistory, resolveMetaHistory } from "./meta-history.js";
...
describe("IncrementalMetaHistory", () => {
	const resolveMetaHistory = (metaHistory) => {
		const imh = new IncrementalMetaHistory();
		for (const op of metaHistory) {
			imh.addOperation(op);
		}
		return imh.getLinearHistory();
	};
	it("should resolve to a linear history without any meta operations", () => {
		expect(resolveMetaHistory([
			{ id: "abc1", metaLevel: 0, type: "line", name: "Draw Line", color: "blue" },
